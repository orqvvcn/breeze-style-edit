...
void Decoration::updateButtonsGeometry()
{
    ...
    // Add padding based on corner radius to avoid clipping by rounded corners
    const int cornerRadiusPadding = s->smallSpacing() * Metrics::Frame_FrameRadius;
    ...
    if (!m_leftButtons->buttons().isEmpty()) {
        ...
        // padding (add corner radius padding)
        const int vPadding = isTopEdge() ? 0 : s->smallSpacing() * Metrics::TitleBar_TopMargin;
        const int hPadding = s->smallSpacing() * Metrics::TitleBar_SideMargin + cornerRadiusPadding;
